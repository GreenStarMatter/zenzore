...
var sampleCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		sig := signal.NewSignal(typ, exp, rand)
		sensor := signal.NewSensor("testSensorSN", "testSensorPN", sig)
		device := signal.NewDevice("testDeviceSN", "testDevicePN")
		device.AddSensor(sensor)
		sig.GenerateSignalSample()
		jsonData := device.ExportDeviceData()
		message := msg.New()
		message.CreatePubSubClient()
		defer message.Client.Close()
		message.AcceptGenericJson(jsonData)
